-- SELECT Some Columns for Statements Requests
...
-- ORDER BY, LIMIT, DISTINCT, Renaming columns Questions Answers
...
-- UPPER, LOWER, LENGTH, TRIM OF DATA
...
-- Returning the email and the length of emails
SELECT
	email,
...
-- Concatenating first and last names to create full names
SELECT
	first_name,
	last_name,
...
-- Some Boolean expressions with (% subString)
...
-- returning only government employees
select
  first_name || ' ' || last_name as full_name,
  email as gov_email
from employees
...
--- Calcuate the following pay Categories
...
-- UNION STATEMENT (to stack data on top each other)
...
--- Average Salaries with Their Differences
...
-- Calculate the total_salary of employees who were hired "within" the 30-day period before the hire_date of the current employee
SELECT
	hire_date,
	salary,
	(
